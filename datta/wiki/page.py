...
def check_lock(wiki, title):
    ...
    if wiki.read_only:
        raise error.ForbiddenErr(_("This site is read-only."))
    if title in restricted_pages:
        raise error.ForbiddenErr(_("""Can't edit this page.
It can only be edited by the site admin directly on the disk."""))
    if title in wiki.index.page_links(wiki.locked_page):
        raise error.ForbiddenErr(_("This page is locked."))
...
def get_page(request, title, wiki=None):
    ...
    if title:
        try:
            page_class, mime = filename_map[title]
        except KeyError:
            mime = page_mime(title)
            major, minor = mime.split('/', 1)
            try:
                page_class = mime_map[mime]
            except KeyError:
                try:
                    plus_pos = minor.find('+')
                    if plus_pos > 0:
                    ...
                    page_class = mime_map[base_mime]
                except KeyError:
                    try:
                        page_class = mime_map[major]
                    except KeyError:
                        page_class = mime_map['']
    else:
...
class WikiPage(object):
    ...
    def __init__(self, wiki, request, title, mime):
        ...
        # if hasattr(request, 'get_url'):
        self.get_url = wiki.get_url
        self.get_download_url = wiki.get_download_url
    ...
    def wiki_link(self, addr, label=None, class_=None, image=None, lineno=0):
        ...
        if parser.external_link(addr):
            ...
            if addr.startswith(':'):
                ...
                href = '/+' + werkzeug.escape(addr[1:], quote=True) + '/'
    ...
    def template(self, template_name, **kwargs):
        # template = self.wiki.jinja_env.get_template(template_name)
        edit_url = None
        if self.title:
            try:
        ...
        html = self.wiki.render_template(template_name, **context)
        return html
...
class WikiPageText(WikiPage):
    ...
    def render_editor(self, preview=None, captcha_error=None):
        ...
        try:
            ...
            comment = _('created')
            rev = -1
        except error.ForbiddenErr as e:
            return werkzeug.html.p(werkzeug.html(str(e)))
        if preview:
            lines = preview
...
class WikiPageWiki(WikiPageColorText):
    ...
    def __init__(self, *args, **kw):
        ...
        if self.config.get('wiki_words', False):
            self.parser = parser.WikiWikiParser
        else:
            self.parser = parser.WikiParser
        if self.config.get('ignore_indent', False):
            try:
    ...
    def view_content(self, lines=None):
        ...
        content = ['<a href="%s"><img src="%s" alt="%s"></a>'
                   % (self.get_url(self.title, 'download'),
                      self.get_url(self.title, 'render'),
